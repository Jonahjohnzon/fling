...
const Forumbody = ({thread, size, index, load}) => {
    ...
    const Left =()=>{
            ...
            const route =`/trending/${postInt - 1}`
             navigate.push(route)
    }
    const Right =()=>{
            const postInt = parseInt(index, 10); 
  ...
  return (
    <div className=' w-[100%] text-white text-sm '>
            ...
            <p  className=' text-xs lg:text-sm font-medium hover:underline cursor-pointer text-white'>Trending</p>
          </div>
        ...
        <section className=' w-full bg-metal rounded-sm'>
            <div>
                <div className=' py-3 px-2 border-b-[1px] bg-textbg border-b-white border-opacity-10'>
                    <p className=' sm:text-base text-sm font-semibold'>TRENDING THREADS</p>
                </div>
            </div>
            <div>
